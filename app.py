...
class PlusBusApp(ctk.CTk):
    ...
    def dibujar_sidebar(self):
        ...
        self.btn_inicio = ctk.CTkButton(self.sidebar, text="🏚 Inicio", font=("arial", 18, "bold"), fg_color="transparent", text_color=COLOR_ORANGE, height=45, anchor="w", command=lambda: self.show_frame("InicioView"))
        ...
        self.btn_pasajes = ctk.CTkButton(self.sidebar, text="🎫 Pasajes", font=("arial", 18, "bold"), fg_color="transparent", text_color=COLOR_WHITE, height=45, anchor="w", command=lambda: self.show_frame("PasajesView"))
        ...
        self.btn_pago = ctk.CTkButton(self.sidebar, text=" 💵 Pagar", font=("arial", 18, "bold"), fg_color="transparent", text_color=COLOR_ORANGE, height=45, anchor="w", command=lambda: self.show_frame("PagoView"))
...
class InicioView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        btn_buscar = ctk.CTkButton(form_card, text="Buscar Flotas Disponibles", font=("Inter", 20, "bold"), fg_color=COLOR_ORANGE,corner_radius=25, text_color=COLOR_WHITE, hover_color="#ea580c", height=45, command=self.buscar_viaje)
...
class PasajesView(ctk.CTkFrame):
    # La vista de pasajes se ha diseñado para mostrar los resultados de la búsqueda rápida realizada en la vista de inicio. Aquí se presentan las flotas disponibles que coinciden con los criterios de búsqueda ingresados por el usuario, mostrando información relevante como la empresa, la ruta, el horario y el precio. Además, se incluye un contador dinámico que muestra la cantidad de flotas encontradas y un mensaje de error si no se encuentran resultados para la ruta seleccionada. Esta página actúa como el punto intermedio entre la búsqueda inicial y la selección del viaje para proceder al pago.
    def __init__(self, parent, controller):
        super().__init__(parent, corner_radius=0, fg_color=COLOR_BG_LIGHT)
        self.controller = controller

        # Estructura de la vista de pasajes: Se ha diseñado una estructura clara y funcional para la vista de pasajes, que incluye un encabezado con el título de la sección y un contador dinámico que muestra la cantidad de flotas encontradas. Debajo del encabezado, se encuentra un contenedor scrollable que muestra las tarjetas de cada viaje disponible según los criterios de búsqueda ingresados por el usuario. Cada tarjeta presenta información relevante sobre el viaje, como la empresa, la ruta, el horario y el precio, y también incluye un botón para seleccionar el viaje y proceder al pago. Si no se encuentran resultados para la ruta seleccionada, se muestra un mensaje de error informativo.
        self.header_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.header_frame.pack(fill=tk.X, padx=30, pady=(30, 10))
        
        # Título principal de la sección, con un estilo destacado para resaltar la importancia de la información presentada y generar una primera impresión sólida en los usuarios. El título indica claramente que se están mostrando los resultados de las salidas disponibles, lo que ayuda a los usuarios a entender rápidamente el propósito de esta sección.
        ...
        #   Contador dinámico de flotas encontradas: Este contador se actualiza automáticamente cada vez que se muestra la vista de pasajes, reflejando la cantidad de flotas disponibles que coinciden con los criterios de búsqueda ingresados por el usuario. El contador está diseñado con un estilo que resalta su función, utilizando colores que contrastan con el fondo para mejorar la visibilidad y facilitar la comprensión de la información presentada.
        ...
        #   Contenedor scrollable para los resultados: Este contenedor se ha diseñado para mostrar las tarjetas de cada viaje disponible de manera organizada y fácil de navegar. El contenedor es scrollable, lo que permite a los usuarios desplazarse por la lista de resultados sin perder la estructura visual. Cada tarjeta dentro del contenedor presenta información relevante sobre el viaje, como la empresa, la ruta, el horario y el precio, y también incluye un botón para seleccionar el viaje y proceder al pago. Esta sección actúa como el punto intermedio entre la búsqueda inicial y la selección del viaje para proceder al pago.
        self.scroll_container = ctk.CTkScrollableFrame(self, fg_color="transparent")
        self.scroll_container.pack(fill=tk.BOTH, expand=True, padx=30, pady=10)
        
        # Al mostrar la vista de pasajes, se ejecuta la función "al_mostrar_vista" que se encarga de actualizar el contenido del contenedor scrollable con los resultados de la búsqueda rápida realizada en la vista de inicio. Esta función filtra los datos de viajes disponibles según los criterios de búsqueda ingresados por el usuario, actualiza el contador dinámico con la cantidad de flotas encontradas y muestra las tarjetas correspondientes a cada viaje disponible. Si no se encuentran resultados para la ruta seleccionada, se muestra un mensaje de error informativo.
    def al_mostrar_vista(self):

        # Limpieza de resultados previos: Antes de mostrar los nuevos resultados de la búsqueda, se eliminan todos los widgets hijos del contenedor scrollable para evitar la acumulación de resultados anteriores y garantizar que solo se muestren los resultados relevantes a la búsqueda actual. Esto asegura una experiencia de usuario limpia y organizada al navegar por los resultados de las salidas disponibles.
        for widget in self.scroll_container.winfo_children():
            widget.destroy()
            
        filtro = self.controller.busqueda_rapida
        
        # Filtrado de viajes según la búsqueda rápida: Se filtran los datos de viajes disponibles en el controlador utilizando los criterios de búsqueda ingresados por el usuario en la vista de inicio. El filtrado se realiza comparando las ciudades de origen y destino de cada viaje con los valores ingresados por el usuario, utilizando una comparación que no distingue entre mayúsculas y minúsculas para mejorar la flexibilidad de la búsqueda. Si no se han ingresado criterios de búsqueda, se muestran todos los viajes disponibles.
        if filtro:
            viajes_filtrados = [
        ...
        # Al mostrar los resultados filtrados, se verifica si la lista de viajes filtrados está vacía. Si no se encuentran flotas para la ruta seleccionada, se muestra un mensaje de error informativo dentro del contenedor scrollable, indicando que no se encontraron resultados para la búsqueda realizada. Esto proporciona una retroalimentación clara al usuario sobre el resultado de su búsqueda y mejora la experiencia de usuario al navegar por los resultados de las salidas disponibles.
        if not viajes_filtrados:
            no_results = ctk.CTkLabel(self.scroll_container, text="No se encontraron flotas para la ruta seleccionada.", font=("Inter", 18), text_color=COLOR_TEXT_MUTED)
            no_results.pack(pady=40)
            return

        # Mostrar tarjetas de viajes disponibles: Para cada viaje que coincide con los criterios de búsqueda, se crea una tarjeta dentro del contenedor scrollable que presenta información relevante sobre el viaje, como la empresa, la ruta, el horario y el precio. Cada tarjeta también incluye un botón para seleccionar el viaje y proceder al pago. El diseño de las tarjetas está pensado para ser visualmente atractivo y fácil de leer, utilizando colores y estilos que resaltan la información importante y facilitan la interacción del usuario.
        for viaje in viajes_filtrados:
            card = ctk.CTkFrame(self.scroll_container, fg_color=COLOR_WHITE, corner_radius=10, border_width=1, border_color="#e2e8f0")
            card.pack(fill=tk.X, pady=8, padx=5)
            
            # Contenedor de información expandible a la izquierda
            info_frame = ctk.CTkFrame(card, fg_color="transparent")
            info_frame.pack(side=tk.LEFT, padx=20, pady=15, fill=tk.BOTH, expand=True)
            info_frame.grid_columnconfigure(0, weight=1) # Asegura la expansión del texto para empujar el botón
            
            ctk.CTkLabel(info_frame, text=viaje["empresa"], font=("Inter", 23, "bold"), text_color=COLOR_BLUE_DARK).grid(row=0, column=0, sticky="w")
            
            # Información de ruta y horario: Se muestra la información de la ruta utilizando un formato claro y visualmente atractivo, con íconos que representan el origen y destino para mejorar la comprensión de la información presentada. Además, se muestra el horario de salida junto con un ícono de calendario para resaltar la importancia de esta información y facilitar la lectura por parte del usuario. Esta sección proporciona detalles clave sobre el viaje, ayudando a los usuarios a tomar decisiones informadas al seleccionar su pasaje.
            ...
            # Información de horario: Se muestra el horario de salida del viaje utilizando un formato claro y visualmente atractivo, con un ícono de calendario para resaltar la importancia de esta información. El horario se presenta junto con la fecha de salida para proporcionar una visión completa del momento en que el viaje está programado, lo que ayuda a los usuarios a planificar su viaje de manera efectiva.
            horario_texto = f"📅 {viaje['fecha']}  |  🕒 Salida: {viaje['hora']}"
            ctk.CTkLabel(info_frame, text=horario_texto, font=("Inter", 16), text_color=COLOR_TEXT_MUTED).grid(row=2, column=0, sticky="w")
            
            #CONTEXTO DE ACCIÓN (DERECHA DE LA TARJETA)
            action_frame = ctk.CTkFrame(card, fg_color="transparent")
            action_frame.pack(side=tk.RIGHT, padx=20, pady=15, fill=tk.Y)
            
            ctk.CTkLabel(action_frame, text=f"Bs {viaje['precio']}", font=("Inter", 20, "bold"), padx=30, text_color=COLOR_ORANGE).pack(anchor="e")
            
            # 1. Creamos la etiqueta de texto para el asiento
            lbl_asiento = ctk.CTkLabel(action_frame, text="Asiento:", font=("Inter", 11, "bold"), text_color=COLOR_TEXT_DARK)
            lbl_asiento.pack(side=tk.LEFT, padx=(0, 5), pady=(5, 0))
            
            # 2. Creamos el ComboBox desplegable con opciones de asientos libres
            combo_asiento = ctk.CTkComboBox(action_frame, values=["1", "2", "3", "4", "5", "10", "12", "15", "20"], width=65, height=28)
            combo_asiento.pack(side=tk.LEFT, padx=(0, 10), pady=(5, 0))
            combo_asiento.set("3") # Valor inicial por defecto
            
            # 3. Vinculamos el botón para que extraiga el valor seleccionado del combo_asiento correspondiente
            btn_comprar = ctk.CTkButton(
                ...
                width=100, 
                height=32, 
                command=lambda v=viaje, c=combo_asiento: self.ir_a_asiento_pagar(v, c.get())
            )
            btn_comprar.pack(side=tk.LEFT, pady=(5, 0))

    def ir_a_asiento_pagar(self, viaje, asiento_elegido):
        # Creamos una copia exacta del viaje seleccionado
        compra = viaje.copy()
        # Inyectamos de forma dinámica el número de asiento seleccionado del menú
        compra["asiento"] = asiento_elegido  
        
        # Guardamos en el estado global y levantamos la interfaz de pagos
        self.controller.compra_pendiente = compra
...
class PagoView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(self, text="Completar Pago Seguro", font=("Inter", 27, "bold"), text_color=COLOR_BLUE_DARK).pack(pady=(30, 10), padx=30, anchor="w")
        
        # Contenedor principal dividido en dos columnas: Este contenedor se ha diseñado para organizar el contenido de la vista de pago en dos columnas principales, una para el formulario de ingreso de datos del pasajero y otra para el resumen del viaje seleccionado. El contenedor tiene un fondo transparente para mantener la consistencia visual con el diseño general de la página, y se ha configurado para llenar todo el espacio disponible dentro de la vista. La estructura de este contenedor permite una presentación clara y organizada de la información, facilitando la navegación y comprensión por parte del usuario.
        ...
        # Contenedor interno del formulario: Este contenedor se ha diseñado para organizar los campos de entrada y la información relacionada con el pago de manera clara y estructurada. El contenedor tiene un fondo transparente para mantener la consistencia visual con el diseño general de la página, y se ha configurado para llenar todo el espacio disponible dentro del card del formulario. Dentro de este contenedor, se encuentran los campos de entrada para el nombre completo y el correo electrónico del pasajero, así como la sección que muestra el método de pago autorizado.
        form_inner = ctk.CTkFrame(form_side, fg_color="transparent")
        form_inner.pack(fill=tk.BOTH, expand=True, padx=25, pady=25)
        
        # Título de la sección de datos del pasajero: Este título se muestra en la parte superior de la sección de formulario, utilizando un estilo destacado para resaltar su importancia y generar una primera impresión sólida en los usuarios. El título indica claramente que se trata de la sección donde se deben ingresar los datos del pasajero, lo que ayuda a los usuarios a entender rápidamente el propósito de esta sección y a visualizar la información que deben proporcionar para completar su compra.
        ctk.CTkLabel(form_inner, text="Datos del Pasajero", font=("Inter", 25, "bold"),text_color=COLOR_BLUE_DARK).pack(anchor="w", pady=(5, 15))
        ctk.CTkLabel(form_inner, text="Nombre Completo", font=("Inter", 18, "bold"), text_color=COLOR_TEXT_DARK).pack(anchor="w", pady=(5, 2))
        self.entry_nombre = ctk.CTkEntry(form_inner, height=38, fg_color=COLOR_BLUE_DARK)
        self.entry_nombre.pack(fill=tk.X, pady=(0, 15))
        
        # Campo de entrada para el correo electrónico, con un diseño que incluye un borde destacado para mejorar la visibilidad y un placeholder que indica claramente que se debe ingresar un correo electrónico válido. Este campo es esencial para establecer un canal de comunicación con el usuario, permitiendo enviar confirmaciones de compra, boletos digitales y cualquier otra información relevante relacionada con su viaje. El diseño del campo está pensado para ser visualmente atractivo y fácil de usar, utilizando colores y estilos que resaltan la importancia de esta información.
        ctk.CTkLabel(form_inner, text="Correo Electrónico", font=("Inter", 18, "bold"), text_color=COLOR_TEXT_DARK).pack(anchor="w", pady=(5, 2))
        self.entry_email = ctk.CTkEntry(form_inner, height=38, fg_color=COLOR_BLUE_DARK)
        self.entry_email.pack(fill=tk.X, pady=(0, 20))
        
        # Método de pago autorizado: En esta sección se muestra el método de pago autorizado para la compra del pasaje, que en este caso es la tarjeta de crédito o débito con procesamiento inmediato. Se utiliza un estilo destacado para resaltar esta información, utilizando colores y tipografía que contrastan con el fondo para mejorar la visibilidad y facilitar la comprensión de la información presentada. Esta sección proporciona claridad a los usuarios sobre el método de pago que se utilizará para completar su compra, lo que ayuda a generar confianza y seguridad en el proceso de pago.
        ctk.CTkLabel(form_inner, text="Método de Pago Autorizado", font=("Inter", 18, "bold"), text_color=COLOR_BLUE_DARK).pack(anchor="w", pady=(5, 8))
        
        self.lbl_metodo_unico = ctk.CTkLabel(form_inner, text="💳  Tarjeta de Crédito / Débito (Procesamiento Inmediato)", font=("Inter", 18, "bold"), text_color=COLOR_BLUE_MEDIUM)
        ...
        # Resumen del viaje seleccionado: En esta sección se muestra un resumen detallado del viaje seleccionado por el usuario, incluyendo información como la empresa, la ruta, el horario y el precio. Este resumen se actualiza dinámicamente cada vez que se muestra la vista de pago, reflejando la información del viaje que el usuario ha seleccionado para comprar. Además, se incluye un mensaje de advertencia si no hay ningún viaje seleccionado, indicando al usuario que debe regresar a la sección de pasajes para elegir una salida antes de proceder con el pago. Esta sección actúa como un boleto digital que proporciona una visión clara y completa del viaje que el usuario está a punto de comprar.
        ...
        # Título del boleto digital: Este título se muestra en la parte superior de la sección de resumen del viaje seleccionado, utilizando un estilo destacado para resaltar su importancia y generar una primera impresión sólida en los usuarios. El título indica claramente que se trata de un boleto digital, lo que ayuda a los usuarios a entender rápidamente el propósito de esta sección y a visualizar la información del viaje que están a punto de comprar.
        self.ticket_titulo = ctk.CTkLabel(ticket_inner, text="🎫 Ticket Digital", font=("Inter", 25, "bold"), text_color=COLOR_WHITE)
        ...
        # Botón de confirmación de pago: Este botón se encuentra debajo del resumen del viaje seleccionado y permite a los usuarios confirmar su transacción para completar la compra de su pasaje. Al hacer clic en este botón, se activa la función "procesar_pago" que se encarga de validar los datos ingresados por el usuario, procesar el pago y generar un boleto digital con un código alfanumérico único. Si no hay ningún viaje seleccionado o si los campos de contacto están incompletos, se muestran mensajes de advertencia o error correspondientes para guiar al usuario en el proceso de compra. Este botón es esencial
        ...
        # Al mostrar la vista de pago, se ejecuta la función "al_mostrar_vista" que se encarga de actualizar el resumen del viaje seleccionado en la sección de boleto digital. Esta función verifica si hay un viaje seleccionado en el estado centralizado del controlador bajo la clave "compra_pendiente". Si hay un viaje seleccionado, se muestra un resumen detallado del viaje, incluyendo la empresa, la ruta, el horario y el precio. Si no hay ningún viaje seleccionado, se muestra un mensaje de advertencia indicando al usuario que debe regresar a la sección de pasajes para elegir una salida antes de proceder con el pago. Además, esta función también actualiza el estado del botón de confirmación de pago, habilitándolo solo cuando hay un viaje seleccionado y deshabilitándolo cuando no hay ningún viaje seleccionado.
    def al_mostrar_vista(self):
        ...
        if compra:
            ...
            # Habilitación del botón de pago: Esta validación garantiza que el proceso de pago solo se realice cuando se cuenta con la información necesaria y un viaje seleccionado. Si hay un viaje seleccionado, el botón de confirmación de pago se habilita, permitiendo al usuario
        else:
            ...
            # Esta validación garantiza que el proceso de pago solo se realice cuando se cuenta con la información necesaria y un viaje seleccionado, mejorando así la experiencia del usuario y evitando
    def procesar_pago(self):
       ...
       # Procesamiento de pago simulado: En esta sección se simula el procesamiento del pago utilizando un método de pago unificado (tarjeta de crédito/débito). Se asume que el pago se procesa correctamente, y se genera un código alfanumérico único para el boleto digital. Este código se utiliza para identificar de manera única cada boleto emitido y se incluye en el mensaje de confirmación que se muestra al usuario después de completar la transacción. Además, se actualiza el estado centralizado del controlador para reflejar la compra realizada, incluyendo la información del pasajero y el código del boleto generado.
        ...
        if "asientos_ocupados" in compra:
            compra["asientos_ocupados"].append(compra.get("asiento", random.randint(1, 40)))
        
        # Inserción del ticket generado dentro del historial dinámico de la sesión
        if not hasattr(self.controller, 'historial_compras'):
            self.controller.historial_compras = []
...
class TurismoView(ctk.CTkFrame):
    ...
    {"titulo": "Sabana Amazónica (Beni)", "desc": "Un paraíso de biodiversidad y llanuras inundables. Ideal para navegar por ríos y observar fauna exótica en su hábitat natural.", "cat": "Llanos", "imagen": "imagenes/beni.jpg"}
        ...
        for dest in destinos:
            ...
            ctk.CTkLabel(top_frame, text=dest["cat"], font=("Inter", 19, "bold"), fg_color=COLOR_BLUE_LIGHT, text_color=COLOR_BLUE_MEDIUM, corner_radius=20, height=20, padx=8).pack(side=tk.RIGHT)
...
class HistorialView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        super().__init__(parent, corner_radius=35, fg_color=COLOR_BG_LIGHT)
        self.controller = controller
        
        # Estructura de la vista de historial de compras: Se ha diseñado una estructura clara y visualmente atractiva para la vista de historial de compras, que incluye un encabezado con un título destacado y una descripción introductoria, seguido de un contenedor scrollable donde se muestran tarjetas detalladas de cada boleto adquirido durante la sesión activa. Cada tarjeta presenta información relevante sobre el boleto, como la empresa, la ruta, el horario, el precio, el asiento seleccionado y el código del boleto, lo que permite a los usuarios revisar fácilmente su historial de compras y mantener un registro centralizado de sus pasajes adquiridos.
        ctk.CTkLabel(self, text="Historial de Boletos Emitidos", font=("Inter", 30, "bold"), text_color=COLOR_BLUE_DARK).pack(pady=(30, 5), padx=30, anchor="w")
        ctk.CTkLabel(self, text="Registro centralizado de pasajes adquiridos durante la sesión activa", font=("Inter", 16), text_color=COLOR_TEXT_MUTED).pack(pady=(0, 15), padx=30, anchor="w")
        
        # Contenedor scrollable para el historial de compras: Este contenedor permite mostrar una lista de boletos adquiridos que puede ser más larga que el espacio visible en la pantalla. Al ser scrollable, los usuarios pueden desplazarse hacia abajo para explorar todo su historial de compras sin que la interfaz se sature, manteniendo una experiencia de usuario fluida y organizada.
        ...
        # Al mostrar la vista de historial de compras, se ejecuta la función "al_mostrar_vista" que se encarga de limpiar recursivamente los elementos visuales previos en el contenedor scrollable para redibujar con datos nuevos. Luego, se verifica si hay alguna compra registrada en el estado centralizado del controlador bajo la clave "historial_compras". Si no hay ninguna compra registrada, se muestra un mensaje informativo dentro del contenedor scrollable indicando que no se registran pasajes adquiridos en este momento. Si hay compras registradas, se despliega visualmente un listado ordenado de boletos adquiridos, mostrando los más recientes primero, con información relevante como la empresa, ruta, horario, precio, asiento seleccionado y código del boleto para cada compra realizada durante la sesión activa.
    def al_mostrar_vista(self):
        ...
        # Validación de existencia de compras: Antes de intentar mostrar el historial de compras, se verifica si hay alguna compra registrada en el estado centralizado del controlador bajo la clave "historial_compras". Si no hay ninguna compra registrada, se muestra un mensaje informativo dentro del contenedor scrollable indicando que no se registran pasajes adquiridos en este momento. Esto proporciona una retroalimentación clara al usuario sobre el estado de su historial de compras y mejora la experiencia de usuario al navegar por esta sección.
        if not compras:
            no_tickets = ctk.CTkLabel(self.scroll_historial, text="📜 No se registran pasajes adquiridos en este momento.", font=("Inter", 18), text_color=COLOR_TEXT_MUTED)
        ...
        for ticket in reversed(compras):
            ...
            ctk.CTkLabel(inner, text=ticket["empresa"], font=("Inter", 24, "bold"), text_color=COLOR_BLUE_DARK).grid(row=0, column=0, sticky="w")
            ctk.CTkLabel(inner, text=ticket["codigo_boleto"], font=("Inter", 19, "bold"), fg_color=COLOR_BLUE_LIGHT, text_color=COLOR_BLUE_MEDIUM, corner_radius=6, height=24, padx=10).grid(row=0, column=1, sticky="e")
            ...
            ctk.CTkLabel(inner, text=detalles_ruta, font=("Inter", 17), text_color=COLOR_TEXT_DARK).grid(row=1, column=0, columnspan=2, sticky="w", pady=(6, 2))
            ...
            ctk.CTkLabel(inner, text=detalles_pasajero, font=("Inter", 17), text_color=COLOR_TEXT_MUTED).grid(row=2, column=0, columnspan=2, sticky="w")
